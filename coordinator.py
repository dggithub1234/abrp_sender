...
class ABRPSenderCoordinator:
    def __init__(self, hass: HomeAssistant, entry):
        ...
        self.interval = timedelta(seconds=entry.options.get(CONF_SCAN_INTERVAL, entry.data.get(CONF_SCAN_INTERVAL, DEFAULT_SCAN_INTERVAL)))
        self._unsub = None
        ...
        async with aiohttp.ClientSession() as session:
            try:
                async with session.post(ABRP_URL, json=payload) as resp:
                    if resp.status != 200:
                        _LOGGER.error("ABRP send failed (%s): %s", resp.status, await resp.text())
                    else:
                        _LOGGER.debug("ABRP data sent successfully: %s", payload)
            except Exception as e:
                _LOGGER.error("Error sending to ABRP: %s", e)
